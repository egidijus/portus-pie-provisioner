...
# (cd base && docker build -t portus_base . )
(cd portus && docker build -t portus . )
(cd registry && docker build -t registry . )
...
setup_database() {
  ...
  while [ $RETRY -ne 0 ]; do
    ...
    # docker-compose run --rm portus rake db:setup
    # &> /dev/null
...
clean() {
  ...
  docker-compose kill
  docker-compose rm nginx portus_web portus_crono registry
}
...
docker-compose restart portus_crono
